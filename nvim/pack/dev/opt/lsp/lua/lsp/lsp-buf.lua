...
local function jump_to_location(item, win, reuse_win)
  ...
  if reuse_win then
    ...
    if w ~= win then api.nvim_set_current_win(w) end
  end
...
function M.get_locations(method, opts)
  ...
  lsp.buf_request_all(
    bufnr,
    method,
    function(client) return util.make_position_params(win, client.offset_encoding) end,
    function(results)
      ...
      for client_id, res in pairs(results) do
        ...
        local locations = {}
        if res then locations = vim.islist(res.result) and res.result or { res.result } end
      ...
      if opts.on_list then return opts.on_list(all_items) end
  ...
  lsp.buf_request_all(bufnr, 'textDocument/references', function(client)
    ...
    if not next(all_items) then
      ...
      if #all_items == 2 then
        ...
        all_items = vim.tbl_filter(
          function(l) return not (curr_line == l.lnum and l.user_data and l.user_data.uri == uri) end,
          all_items
        )
      end
